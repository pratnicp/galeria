...
//    echo "<h1>Artyści</h1>";
    ...
    for ($index = 0; $index < count($artists); $index++) {
        if ($new_row && $index==0) {
            echo '<td rowspan="2"><h1>Artyści</h1></td>';
        } else if($new_row && $index >2) {
            echo '<td></td>';
        }
        $new_row=false;
        /* @var $artist Artist */
        ...
        $link = generate_link("index.php?action=paintings&amp;artist=".$artist->get_id()."&amp;store=default");
        echo "<td><a href=\"$link\">$name</a></td>";
        if(($index % 2 == 1) && ($index+1<count($artists))) {
            echo '</tr><tr>';
            $new_row=true;
        }
    }
...
#    echo "<p>".$artist->get_born()." <a href=\"$link\">biografia</a></p>";
...
function admin_object($object, $conn) {
    switch ($object) {
        ...
        case 'artist':
            ...
            if(!$new) {
                $paintings = Painting::load($conn, "artist_id=$artist_id");
                echo '<h1>Obrazy</h1>';
                list_paintings($paintings, $artist_id, true, true, true);
            }
            break;
        case 'painting':
            generate_admin_subnavigation(true);
